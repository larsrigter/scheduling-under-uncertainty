...
class ReinforcementLearning:
    def __init__(self,gamma=0.95,decay=30000,learnRate=0.0001,
                 ...
                 saveNorm=2016,qUpdateFreq=2,updatePol=2000,saveFiles=None,normalizer=None):
        self.setup=setup
    ...
    def load_target_qModel(self):
        self.target_qModel.load_state_dict(torch.load(self.saveFiles['critic']+'.mod'))
    ...
    def save_actor(self,e,reward=None):
        if type(reward)!=type(None):
            ...
            torch.save(self.policy.state_dict(), self.saveFiles['actor']+'.mod')
    ...
    def save_critic(self,e=None,reward=None):
        if type(reward)!=type(None):
            ...
            torch.save(self.qModel.state_dict(), self.saveFiles['critic']+'.mod')

    def load_actor(self):
        path=self.saveFiles['actor']+'.mod'
        self.policy.load_state_dict(torch.load(path))
        
    def load_critic(self):
        path=self.saveFiles['critic']+'.mod'
        ...
        if len(pendingJobs)>0:
            ...
            while True and len(pendingJobs)>0:
                ...
                if charged<=env.demand[time]:
                    ...
                    self.states=torch.cat([self.states[:action], self.states[action+1:]])
...
#                 print(self.oldStates,self.reward,self.states)
    ...
    def actor_critic_policy(self,env,pendingJobs,time,intervalLength,ite,e,episodeStep,
                        status):
        if status=='train':
            #load target model
            if (ite+1)%self.tau==0:
